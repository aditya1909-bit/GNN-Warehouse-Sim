...
def _write_dispatch_decision_explainer(figures_dir: Path, rows: list[dict[str, object]]) -> dict[str, Path]:
    ...
    target_row = next((row for row in dispatch_rows if "due_pressure" in str(row.get("scenario_id"))), None)
    ...
    candidate_groups = [group for group in grouped.values() if len(group) >= 3]
    if not candidate_groups:
        return {}
    selected_group = max(candidate_groups, key=lambda group: _dispatch_group_spread(group))
    for row in selected_group:
        row["_explanation_score"] = str(_dispatch_explanation_score(row))

    plot_path = figures_dir / "dispatch_decision_explainer.png"
    plt = _load_matplotlib_pyplot()
    ordered_rows = sorted(selected_group, key=lambda row: float(row["_explanation_score"]), reverse=True)[:8]
    labels = [
        f"{row['candidate_robot_id']} -> {row['candidate_task_id'] or row['candidate_charging_node_id']}"
        for row in ordered_rows
    ]
    scores = [float(row["_explanation_score"]) for row in ordered_rows]
    ...
    plt.xlabel("Composite urgency-efficiency score")
    plt.title(
...
def _plot_bottleneck_occupancy(axis, *, baseline_traces: list[dict[str, str]], challenger_traces: list[dict[str, str]]) -> None:
    baseline_series = _occupancy_series(baseline_traces)
    challenger_series = _occupancy_series(challenger_traces)
    ...
    axis.set_ylabel("Traversals touching bottleneck")
...
def _occupancy_series(trajectory_rows: list[dict[str, str]]) -> dict[str, list[float]]:
    if not trajectory_rows:
    ...
    for row in trajectory_rows:
        ...
        if start_time is None or end_time is None:
            continue
        if "r2_c2" not in {source_id, target_id} and "r2_c1" not in {source_id, target_id} and "r2_c3" not in {source_id, target_id}:
            continue
...
def _dispatch_group_spread(group: list[dict[str, str]]) -> float:
    scores = [_dispatch_explanation_score(row) for row in group]
